...
public class KThread {
	...
	// Get the threads waiting for the currentThread to finish
	ThreadQueue currQueue = currentThread.queue;
	// if currQueue is not null, there are threads waiting to join
	if(currQueue != null) {
		// Get the first thread from queue
		KThread curThread = currQueue.nextThread();
		// While there are more threads, add them to ready queue
		while ( curThread != null ) {
			curThread.ready();
			currentThread.queue.acquire(curThread);
			// Get the next thread to ready queue
			curThread = currQueue.nextThread();
		}
	}
    ...
    public static ThreadQueue queue = ThreadedKernel.scheduler.newThreadQueue(true);;
}
